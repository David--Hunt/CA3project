...
if len(sys.argv) == 2 and sys.argv[1] == 'new':
    morph = dlmorph.SWCFileSimplifiedMorphology(swc_filename)
else:
    morph = NrnFileMorphology(swc_filename)
...
default_params = {'cm': cm, 'g_pas': 1./Rm, 'e_pas': El}
responses = step_protocols.run(cell_model=CA3_cell, param_values=default_params, sim=nrn)

plt.plot(responses['step.soma.v']['time'],responses['step.soma.v']['voltage'],'k')
plt.show()
